...
class NVDCompleteService:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    db.commit()
                    logger.info(f"Processed {len(vulnerabilities)} CVEs (Total: {total_processed}/{total_results})")
                    
                    # Save checkpoint
                    if checkpoint_file:
                        self._save_checkpoint(checkpoint_file, params['startIndex'] + len(vulnerabilities))
    ...
    def _process_cve(self, db: Session, vuln_data: Dict[str, Any]) -> None:
        ...
        if existing:
            ...
            existing.affected_products = list(set((existing.affected_products or []) + affected_products))
            ...
            vulnerability = Vulnerability(
                ...
                sources=["nvd"],
                priority_score=self._calculate_priority(cvss_score, severity, published),
