...
class Vulnerability(Base):
    ...
    severity = Column(String(20), nullable=True, index=True)  # CRITICAL, HIGH, MEDIUM, LOW, UNKNOWN
    ...
    # Relationships
    techniques = relationship("Technique", secondary=vulnerability_technique, back_populates="vulnerabilities")
    
    # Indexes for full-text search (PostgreSQL specific)
    __table_args__ = (
        Index('ix_vuln_cve_id_trgm', 'cve_id', postgresql_using='gin', postgresql_ops={'cve_id': 'gin_trgm_ops'}),
        Index('ix_vuln_title_trgm', 'title', postgresql_using='gin', postgresql_ops={'title': 'gin_trgm_ops'}),
        Index('ix_vuln_description_trgm', 'description', postgresql_using='gin', postgresql_ops={'description': 'gin_trgm_ops'}),
    )
...
class Technique(Base):
    ...
    # Relationships
    vulnerabilities = relationship("Vulnerability", secondary=vulnerability_technique, back_populates="techniques")
...
class IOC(Base):
    ...
    # IOC details
    ioc_type = Column(String(50), nullable=False, index=True)  # url, domain, ip, hash, email
    ...
    # Indexes
    __table_args__ = (
        Index('ix_ioc_value_trgm', 'value', postgresql_using='gin', postgresql_ops={'value': 'gin_trgm_ops'}),
    )
...
class SearchCache(Base):
    ...
    # Query details
    query_type = Column(String(50), nullable=False)  # vulnerability_search, ioc_search, etc.
