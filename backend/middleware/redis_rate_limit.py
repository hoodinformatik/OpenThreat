    ...
    def check_rate_limit(self, ip: str) -> Tuple[bool, str, dict]:
        ...
        try:
            ...
            if minute_count > self.requests_per_minute:
                return False, f"Rate limit exceeded: {self.requests_per_minute} requests per minute", rate_limit_info
            
            if hour_count > self.requests_per_hour:
                return False, f"Rate limit exceeded: {self.requests_per_hour} requests per hour", rate_limit_info
...
async def redis_rate_limit_middleware(request: Request, call_next):
    ...
    # Check rate limit
    is_allowed, error_message, rate_info = redis_rate_limiter.check_rate_limit(client_ip)
