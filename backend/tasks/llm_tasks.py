...
def process_llm_queue(batch_size: int = 10, priority: str = "high") -> dict:
    ...
    try:
        ...
        query = db.query(Vulnerability).filter(Vulnerability.llm_processed == False)
        ...
        logger.info(
            f"Queued {processed} CVEs for LLM processing (priority: {priority})"
        )
        return {
            "status": "success",
            "processed": processed,
...
def process_new_cves() -> dict:
    ...
    try:
        ...
        if not vulns:
            ...
            return {"status": "success", "processed": 0, "message": "No new CVEs"}
...
def get_llm_stats() -> dict:
    ...
    try:
        ...
        processed = (
            db.query(Vulnerability).filter(Vulnerability.llm_processed == True).count()
        )
